...
- [ ] Activity indicator on the chat UI — when claude is in the middle of a turn (generating text, running tools, processing), show a subtle indicator (e.g. pulsing dot or small spinner near the status pill) so a user glancing at the web page knows "stuff is happening." Deliberately *no* details — those live in the terminal; the web indicator is just presence/absence. Mechanism: the transcript watcher sets `lastActivityAt = Date.now()` on every new JSONL entry; the UI shows the indicator while `now - lastActivityAt < ~3s` (and arguably a separate "definitely still working" signal during long tool calls between text blocks). Use case: claude is mid long dev sequence and the user is checking the web tab to see if it's still alive.
- [ ] Opt-in completion chime — small bell icon next to the activity indicator. Click to arm a one-shot sound that plays the next time the assistant turn finishes (no pending tool calls / no activity for ~3s). Auto-disarms after firing. Use case: kick off a long task, click the bell, walk away — get a ding when claude is done. Preferred over an always-on chime that the user has to mute, since the default-off respects focus.
