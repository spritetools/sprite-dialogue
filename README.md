...
Exit and relaunch Claude with the channel flag (required because Claude Code's [channels API](https://code.claude.com/docs/en/channels-reference) is still in preview):

```
claude --dangerously-load-development-channels plugin:sprite-dialogue@sprite-dialogue
```

The UI URL is written to `/tmp/sprite-dialogue-url`. Open it in your laptop browser — `sprite console` auto-forwards bound ports.
...
The server picks a port deterministically from the Sprite's hostname (range 30000–39999), so multiple Sprites running sprite-dialogue don't collide on `localhost` when forwarded to your laptop. To override the port explicitly, set `SPRITE_DIALOGUE_PORT` before launching Claude.
