...
const mcp = new Server(
  { name: 'sprite-dialogue', version: '0.1.0' },
  {
    ...
    instructions: [
      'The sender is viewing the sprite-dialogue web UI in their browser, not this terminal session.',
      'Anything you want them to see must go through the reply tool — your transcript output never reaches the UI.',
      '',
      'Messages from the UI arrive as <channel source="sprite-dialogue" chat_id="web" message_id="...">.',
      'If the tag has a file_path attribute, Read that file — it is a screenshot or image upload from the user.',
      'Reply with the reply tool. To include images, save them to a file and pass the path in the files array.',
      '',
      `The UI is accessible on port ${PORT} (forward via "sprite proxy ${PORT}" from a local machine).`,
    ].join('\n'),
  },
)
